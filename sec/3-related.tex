...
\{How was it done in the very beginning\}
	% The extraction of "area of interest" in a fingerprint image has been previously explored using "adaptive thresholding" \cite{zhang2013adaptive}. The threshold method was dynamically adapted based on the image provided. The idea was to find the threshold value for each pixel based on the local mean and standard deviation of the pixel, resulting in a well-defined ROI for an image, but it was limited to only a few images, as the threshold value could not be generalized.
